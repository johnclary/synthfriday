...
document.querySelector('#play').addEventListener('click', function() {
    ...
    //get the loop going...may need d3 timer cause you can't figure out what's broke
    //re-pick better letters
    //
    //future: flickering, scrolling, rainbow, blackout background....play with different back/foreground durations
    //
    //letters!
    ...
    var letterRows = 10; //based on the way the letters have been created in the letter picker app.
    var letterColumns = 7; //letters are 6 circle-columns wide w/ one blank column for spacing
    var word1Length = 5, word2Length = 6;  
    ...
    var delay = 50; //per circle/column duration in ms...aka 'speed'
    var textDuration = 5000;
    var backgroundDuration = 5000;
    //
    var colorScale = d3.scale.linear().range([0,255]).domain([0,letterList.length*letterColumns]); //new linear scale with the range as the hsl range for hue (0 - 255), the domain will be based on the width of the text
    var hue =  Math.floor(Math.random() * 361); //hsl colors: between 0 and 255
    var textHue = hue + 100;
    ...
    r = Math.min(maxRhorizontal,maxRvertical);
    //
    var dia = r*2, totalCol = w/dia, totalRow = h/dia;
    ...
    //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    svg = d3.select("body").append("svg").attr("width", dia * totalCol).attr("height", dia * totalRow)

    for (var i = 0; i < totalCol; i++) {  //for every column
        var g = svg.append("g").attr("class", "x_" + i)
        for (var q = 0; q < totalRow; q++) {  //for every row
            g.append("circle").attr("class", "x_" + i + "_y_" + q + " x_" + i + " disabled") //add a circle with a class that corresoponds to its column/row
    ...
    for (var i = 0; i < absLetters.length; i++) { //for all the letter circles
        d3.selectAll(".x_" + absLetters[i][0] + "_y_" + absLetters[i][1]).classed("active", true).classed("disabled", false); //select them by their col/row position and style them
    }

    fadeIn();

    function fadeIn(){  //this works; it's what you want; and it's slow...could be less memry to build an if statement on the fill rather than simultaneous for active and disabled?...will have to be if you want individual randomness, like before, which is the bread and butter;
            
        textHue = (hue + 180 < 360) ? hue + 180: hue + 180 - 360;
        for (i = 0; i < totalCol; i++) {
            textHue = textHue + 1; //<< taste the rainbow!
            svg.selectAll(".x_" + i).filter(".active").transition().duration(textDuration).delay(i * delay).attr("fill", function(){
                lightness = .5;
        ...
        hue = (hue + 57< 360) ? hue + 57: hue + 57 - 360;
    }
    ...
    function generateAbsPositions(){ //take letter circle position and convert to row/col on screen
        var letterPositions = []
        //first word
        for (var z = 0; z < letterList.length; z++){ //for every letter
            var length = letterList[z].length;
            for (var q = 0; q < length; q++){ //for every circle in letter
                letterPositions.push([ letterList[z][q][0] + (z * letterColumns) + xOffset, letterList[z][q][1] + yOffset ]);  // simplified: [(diameter * letter position) + ( diamater * circle position) + offset, letter position + offset]
            }
        }
        //second word
        for (var z = 0; z < letterList2.length; z++){ //for every letter
    ...
    d3.selectAll(".active").on("mouseover", function(){
        ...
        d3.select(this).style("fill", newColor).attr("r", r/1.5).transition().duration(500).style("fill", color).attr("r", r);
    })
    ...
    var frequencyData = new Uint8Array(john[0].length); //<this denominator "groups" the columns together when assinging the data later
    var newData = [];
    var stupid = -90;
    ...
    function renderChart() {
        ...
        if (stupid == 90) { //60 fps, so this would be every 2 sec...i think
            fadeIn();    //<< loop your fadeIn here if you're into that kinda thing
            stupid = 0;
        }
        ...
        john.data(frequencyData).attr("r", function(d) {
            return scale(d) * r > 0  ? scale(d) * r : 0;
        })
    } 
